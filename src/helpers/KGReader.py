...
class KGReader(BaseReader):
    ...
    def _construct_kg(self) -> NoReturn:
        ...
        self.n_relations = len(self.relations) + 1 if not self.latent_relation_num else len(self.relations) + 1 + self.latent_relation_num
...
if __name__ == '__main__':
    ...
    if "latent_relation_num" in args:
        corpus_path = os.path.join(args.path, args.dataset, f'KGReader_{args.latent_relation_num}.pkl')
    else:
