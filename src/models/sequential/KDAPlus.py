...
class KDAPlus(SequentialModel):
    reader = 'DFTReader'
    # extra_log_args = ['num_layers', 'num_heads', 'gamma', 'alpha', 'freq_rand', 'include_val', 'latent_relation_num', 'include_lrd', "message"]
    extra_log_args = ['num_layers', 'num_heads', 'freq_rand', 'alpha', 'lamda', 'latent_relation_num', 'leave_one_latent', 'include_lrd', "message"]
    ...
    def parse_model_args(parser):
        ...
        parser.add_argument('--latent_relation_num', type=int, default=8,
                            help='the number of latent relations')
        parser.add_argument('--leave_one_latent', type=int, default=1,
    ...
    def forward(self, feed_dict):
        ...
        if self.only_predict:
            # his_i_ids, target_attention = self.lrd_predict(feed_dict)
            # prediction, _, _ = self.rec_forward(feed_dict)
        ...
        if self.only_predict:
            out_dict["lrd_result"] = self.lrd_predict(feed_dict)
